    ...
    private fun goRegister() {
        ...
        mAuth.createUserWithEmailAndPassword(email, password)
            .addOnFailureListener {
                ...
                submitUserInfoToFirebase(name, userName,email, password)
                startActivity(Intent(activity, LoginActivity::class.java))
            }.addOnCanceledListener {
                Toast.makeText(context, "Register Canceled ", Toast.LENGTH_SHORT).show()
            }

        //val acc = Account(email, password, name, userName)
        //dbRef.child("Account").push().setValue(acc)
    }

    private fun submitUserInfoToFirebase(name: String, userName: String, email: String, pass: String) {

        val user = User(name, userName, mAuth.currentUser?.uid!!, email, pass)
